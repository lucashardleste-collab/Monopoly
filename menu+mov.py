...
# Game Variables (Atualizado para a resolução real do tabuleiro)
GAME_WIDTH = 1920
GAME_HEIGHT = 1040
...
# --- RECURSOS DO MENU ---
caminho_menu = os.path.join("images", "menu_technoply.png")
try:
...
# Retângulos de colisão recalculados e centralizados para a resolução de 1920x1040
# Se os botões da imagem ficarem deslocados, altere o segundo número (Y) destas linhas:
# Retângulos com largura e altura maiores para cobrir o botão inteiro
retangulo_iniciar = pygame.Rect(650, 560, 625, 130)  # X ajustado para manter centralizado com a nova largura
retangulo_sair = pygame.Rect(650, 720, 625, 130)     # X ajustado para manter centralizado com a nova largura

# --- RECURSOS DO JOGO (MONOPOLY CORE) ---
game = Monopoly([
    "Capivara",
    "Urso",
    "Cristo",
...
def atualizar_posicoes():
    ...
    # Evita sobreposição visual no mesmo quadrado
    player2.x += 15
    player3.y += 15
    player4.x += 15
    player4.y += 15
...
def draw_game():
    """Renderiza os componentes ativos do tabuleiro de jogo."""
    window.blit(frames[int(current_frame)], (0, 0))

    # Marcadores de base coloridos
    pygame.draw.circle(window, (255, 0, 0), (player1.centerx, player1.centery), 12)
    pygame.draw.circle(window, (0, 0, 255), (player2.centerx, player2.centery), 12)
    pygame.draw.circle(window, (0, 255, 0), (player3.centerx, player3.centery), 12)
    pygame.draw.circle(window, (255, 255, 0), (player4.centerx, player4.centery), 12)
...
def draw_menu():
    """Renderiza os componentes estéticos do menu principal."""
    if imagem_menu:
        window.blit(imagem_menu, (0, 0))
    else:
        window.fill((15, 15, 30))

    # Efeito de Hover (Luz dourada ao passar o mouse)
...
while True:
    ...
    # 1. CAPTURA DE EVENTOS GLOBAL
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            exit()

        # Eventos exclusivos da tela de MENU
        if estado_tela == "MENU" and event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:  # Clique esquerdo
                if retangulo_iniciar.collidepoint(event.pos):
                    estado_tela = "JOGO"  # Inicializa a transição para a partida
                elif retangulo_sair.collidepoint(event.pos):
                    pygame.quit()
                    exit()

        # Eventos exclusivos da tela de JOGO
        if estado_tela == "JOGO" and event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                if passos_restantes == 0:
                    ...
                    print(f"Turno: {game.jogadores[jogador_atual].nome}")
                    print(f"Andará {passos_restantes} casas")

    # 2. SELETOR DE EXECUÇÃO POR ESTADO DE TELA
    if estado_tela == "MENU":
        draw_menu()
        ...
        if tempo_movimento > 1:
            ...
            if passos_restantes > 0:
                game.jogadores[jogador_atual].mover(1, len(CASAS))
                passos_restantes -= 1
                ...
                if passos_restantes == 0:
                    ...
                    print(f"Próximo jogador: {game.jogadores[jogador_atual].nome}")
                    print(f"Posição: {game.jogadores[0].posicao}")
    ...
    # 3. ATUALIZAÇÃO DA TELA DO SISTEMA
